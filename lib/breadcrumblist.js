...
module.exports = (hexo, option) => {
  ...
  if(hexo.is_archive()) {
    ...
    if(hexo.is_year()) {
      let dtf = new Intl.DateTimeFormat(config.language, {year: "numeric", timeZone: config.timezone});
      breadcrumb.itemListElement.push({
        "@type": "ListItem",
        "position": 3,
        "item": {
    ...
    if(hexo.is_month()) {
      let dtf = new Intl.DateTimeFormat(config.language, {month: "numeric", timeZone: config.timezone});
      breadcrumb.itemListElement.push({
        "@type": "ListItem",
        "position": 4,
        "item": {
