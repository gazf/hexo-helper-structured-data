...
module.exports = (hexo, option) => {
  ...
  if(hexo.is_archive()) {
    ...
    breadcrumb.itemListElement.push({
      ...
      "item": {
        ...
        "name": option.label.archive
      }
    });

    if(hexo.is_year()) {
      const dtf = new Intl.DateTimeFormat(config.language, {year: "numeric", timeZone: config.timezone});
      breadcrumb.itemListElement.push({
        "@type": "ListItem",
        "position": 3,
        "item": {
    ...
    if(hexo.is_month()) {
      const dtf = new Intl.DateTimeFormat(config.language, {month: "numeric", timeZone: config.timezone});
      breadcrumb.itemListElement.push({
        "@type": "ListItem",
        "position": 4,
        "item": {
  ...
  if(hexo.is_post()) {
    page.categories.data.forEach(
      (category) => breadcrumb.itemListElement.push({
        "@type": "ListItem",
        "position": 2,
        "item": {
